...
function unlockCourses() {
  cursos.forEach(c => {
    ...
    if (c.prereqs.length && el.classList.contains('hidden')) {
...
cursos.forEach(c => {
  ...
  if (c.prereqs.length) d.classList.add('hidden');

  container.appendChild(d);
});

// 6) Click → tachar/desbloquear
container.addEventListener('click', e => {
  const t = e.target;
  if (!t.classList.contains('course') || t.classList.contains('hidden')) return;

  const code = t.dataset.code;
  const done = t.classList.toggle('completed');
  if (done) selected.add(code);
  else {
